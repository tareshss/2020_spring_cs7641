...
def graph_GA():
    ...
    plt.savefig('./graphs/' + 'Knapsack_GA' + '_' + str(timestr) + '.png')
    plt.close()
...
def graph_MIMIC():
    ...
    ax.set_xlabel("Iteration")
    ax.set_ylabel("Best Fitness")
    title = f"Knapsack MIMIC"
    plt.title(title, fontsize=14, y=1.03)
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('./graphs/' + 'Knapsack_MIMMIC' + '_' + str(timestr) + '.png')
    plt.close()
...
def graph_SA():
    ...
    data_frames = [df_temp_1, df_temp_10, df_temp_50, df_temp_100, df_temp_250, df_temp_500,
    ...
    ax.set_ylabel("Best Fitness")
    timestr = time.strftime("%Y%m%d-%H%M%S")
    title = f"Knapsack for SA"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'Knapsack_SA' + '_' + str(timestr) + '.png')
    plt.close()


def graph_RHC():
    df = pd.read_csv("./output/Knapsack_RHC/rhc__Knapsack_RHC__run_stats_df.csv")
    df_temp_25 = df.loc[(df['Restarts'] == 25) & (df['current_restart'] == 14)].copy()
    df_temp_25 = df_temp_25[['Iteration', 'Fitness']]
    df_temp_25.rename(columns={'Fitness': 'Restarts 25'}, inplace=True)
    df_temp_75 = df.loc[(df['Restarts'] == 75) & (df['current_restart'] == 75)].copy()
    ...
    ax.set_ylabel("Best Fitness")
    timestr = time.strftime("%Y%m%d-%H%M%S")
    title = f"Knapsack for RHC"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'Knapsack_RHC' + '_' + str(timestr) + '.png')
    plt.close()


def graph_timings():
    df_1 = pd.read_csv("./output/Knapsack_GA/ga__Knapsack_GA__run_stats_df.csv")
    df_2 = pd.read_csv("./output/Knapsack_MIMIC/mimic__Knapsack_MIMIC__run_stats_df.csv")
    df_3 = pd.read_csv("./output/Knapsack_SA/sa__Knapsack_SA__run_stats_df.csv")
    df_4 = pd.read_csv("./output/Knapsack_RHC/rhc__Knapsack_RHC__run_stats_df.csv")

    df_1_Best = df_1.loc[(df_1['Population Size'] == 250) & (df_1['Mutation Rate'] == 0.03)].copy()
    df_1_Best = df_1_Best[['Iteration', 'Time']]
    df_1_Best.rename(columns={'Time': 'GA'}, inplace=True)

    df_2_Best = df_2.loc[(df_2['Population Size'] == 150) & (df_2['Keep Percent'] == 0.2)].copy()
    ...
    df_4_Best = df_4.loc[(df_4['Restarts'] == 25) & (df_4['current_restart'] == 14)].copy()
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    title = f"Knapsack Timings for All 4 Algorithms"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'Knapsack_Timings' + '_' + str(timestr) + '.png')
    plt.close()
