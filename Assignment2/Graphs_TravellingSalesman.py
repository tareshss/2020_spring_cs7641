...
def graph_GA():
    ...
    plt.savefig('./graphs/' + 'TSP_GA' + '_' + str(timestr) + '.png')
    plt.close()


def graph_MIMIC():
    df = pd.read_csv("./output/TSP_Maximize_MIMIC/mimic__TSP_Maximize_MIMIC__run_stats_df.csv")
...
def graph_SA():
    df = pd.read_csv("./output/TSP_Maximize_SA/sa__TSP_Maximize_SA__run_stats_df.csv")
    ...
    title = f"TSP for SA"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'TSP_SA' + '_' + str(timestr) + '.png')
    plt.close()
...
def graph_RHC():
    ...
    title = f"TSP for RHC"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'TSP_RHC' + '_' + str(timestr) + '.png')
    plt.close()
