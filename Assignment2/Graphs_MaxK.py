...
def graph_GA():
    df = pd.read_csv("./output/Max_K_GA/ga__Max_K_GA__run_stats_df.csv")
    ...
    plt.savefig('./graphs/' + 'Max_K_GA' + '_' + str(timestr) + '.png')
    plt.close()


def graph_MIMIC():
    df = pd.read_csv("./output/Max_K_MIMIC/mimic__Max_K_MIMIC__run_stats_df.csv")
    ...
    plt.title(title, fontsize=14, y=1.03)
    timestr = time.strftime("%Y%m%d-%H%M%S")
    plt.savefig('./graphs/' + 'Max_K_MIMMIC' + '_' + str(timestr) + '.png')
    plt.close()


def graph_SA():
    df = pd.read_csv("./output/Max_K_SA/sa__Max_K_SA__curves_df.csv")
    ...
    title = f"Max_K SA"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'Max_K_SA' + '_' + str(timestr) + '.png')
    plt.close()


def graph_RHC():
    df = pd.read_csv("./output/Max_K_RHC/rhc__Max_K_RHC__curves_df.csv")
    ...
    title = f"Max_K RHC"
    plt.title(title, fontsize=14, y=1.03)
    plt.savefig('./graphs/' + 'Max_K_RHC' + '_' + str(timestr) + '.png')
    plt.close()
...
def graph_timings():
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    title = f"GA Timings for All 4 Algorithms For 50 Cities"
    plt.title(title, fontsize=14, y=1.03)
